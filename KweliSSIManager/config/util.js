...
async function getPoolGenesisTxnPath(poolName) {
    ...
    const path = `${os.homedir}/.tmp/indy/${poolName}.txn`
    log('Path => ' + path)
    if (!fs.existsSync(path)) {
        await savePoolGenesisTxnFile(path)
    } else {
        log('[util-getPoolGenesisTxnPath] file already exists');
    }
    return path
}

async function poolGenesisTxnData() {
    const poolIp = process.env.TEST_POOL_IP || "127.0.0.1"
...
async function savePoolGenesisTxnFile(filePath) {
    const data = await poolGenesisTxnData()
...
async function mkdir(filePath) {
    return new Promise((resolve, reject) => {
        ...
        log('To make directory at\t' + folderPath)
...
function printMap(preamble, mymap) {
    for ( let entry of mymap.entries() ) {
        let key = entry[0],
            val = entry[1];
        console.log(preamble + key + ' -> ' + val);
    }
}

function isEmptyOrSpaces(str){
    str = str.trim();
    //console.log('[isEmptyOrSpaces] -->' + JSON.stringify(str) + '<--')
    //console.log('[isEmptyOrSpaces] -->' + (str===null||str==='') + '<--')
    return str === null || str === '';
}
